...
import { p as parseAmazonUrl, i as isValidAsin } from "../../../chunks/amazon_WxRnAz-f.mjs";
import { i as isPaapiConfigured, g as getProductByAsin } from "../../../chunks/amazon-paapi_UW5jK4xx.mjs";
import { s as siteConfig } from "../../../chunks/site-config_BzdwJVhh.mjs";
import { renderers } from "../../../renderers.mjs";
function generateAffiliateUrlFromConfig(asin, lang) {
...
function cleanText(text) {
  if (!text) return null;
  return text.replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/\s+/g, " ").trim() || null;
}
...
function paapiToScrapedFormat(paapiData) {
  const fieldStatus = {
...
async function scrapeAmazonProduct(asin, marketplace) {
  const fieldStatus = {
    title: false,
    brand: false,
    price: false,
  ...
  try {
    ...
    if (images.length === 0) {
      const mainImgMatch = html.match(/<img[^>]*id="landingImage"[^>]*src="([^"]+)"/i) || html.match(/<img[^>]*class="[^"]*a-dynamic-image[^"]*"[^>]*src="([^"]+)"/i);
      if (mainImgMatch) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        ...
        message: hasData ? `Product data fetched via ${dataSource === "paapi" ? "Amazon API" : "web scraping"} (${successCount}/${totalFields} fields). Review and complete missing data.` : "Could not fetch product data automatically. Please fill in manually.",
        data: productData
      }),
